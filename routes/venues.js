...
router.get('/', function(req, res) {
  venueCRUD.readVenues().then(response => res.status(200).json(response)).catch(err => res.status(500).json(err));
});
...
router.delete('/:venueID', function(req, res){
  venueCRUD.deleteVenue(req.params.venueID).then(response => res.status(200).json(response)).catch(err => res.status(500).json(err));
});
